...
fun! s:convert(options) "{{{
    ...
    if o_ft=='pdf'
        ...
        if executable('latexmk')
            call s:sys( 'cd '.out_path.' && latexmk -xelatex -shell-escape -f -silent -interaction=nonstopmode '.shellescape(output) )
        elseif executable('pdflatex')
            call s:sys( 'cd '.out_path.' && pdflatex -shell-escape -interaction batchmode '.shellescape(output) )
